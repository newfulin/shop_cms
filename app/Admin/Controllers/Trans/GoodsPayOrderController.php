...
use App\Admin\Models\GoodsPayOrder;

use App\Http\Controllers\Controller;
use Encore\Admin\Controllers\ModelForm;
use Encore\Admin\Layout\Content;

class GoodsPayOrderController extends Controller
{
    use ModelForm;
    public function index()
    {
        return Admin::content(function (Content $content){
            $content->header('商品订单支付流水');
            $content->description('商品订单支付流水');
            $content->body($this->grid());
        });
    }
    public function grid()
    {
        return Admin::grid(GoodsPayOrder::class,function(Grid $grid) {
            $grid->model()->orderBy('create_time', 'desc');
            $grid->column('id','明细流水ID');
            $grid->column('sign','签名')->display(function($text) {
...
//            $grid->column('body','商品描述');
            ...
            $grid->column('prepayid','交易标识')->display(function($text){
                return str_limit($text,10,'....');
            });
